...
User invokes `/resume-tailor-skill` with their materials:
...
Also activates on: "帮我改简历", "tailor my resume for this", "针对JD修改", "匹配岗位要求", "改简历", "优化简历"
...
### Phase 0: Gather Inputs (MANDATORY — run BEFORE any analysis)

**The skill CANNOT proceed without two things from the user. Ask proactively.**

Check what the user has provided. If anything is missing, ask clearly and specifically:

```
我还需要两份材料才能开始：

1. 📋 岗位描述（JD）— 你要投的岗位，复制粘贴全文，或发文件路径
2. 📄 你的简历 — 原始简历文件路径（DOCX/PDF），或直接粘贴内容
...
- If JD is missing: "把你要投的岗位描述（JD）粘贴过来，包含岗位职责和任职要求两部分。"
- If resume is missing: "把你的简历文件路径发我（比如 C:\\Users\\...\\简历.docx），或者直接把简历内容粘贴过来。"
- If both are provided but vague: "这个JD看起来不完整，有没有包含任职要求的部分？另外你的简历方便发文件路径吗，我需要提取原文来确保替换时能找到精确的文字。"

**Once BOTH are received, confirm and proceed:**
...
Then immediately move to Phase A.
